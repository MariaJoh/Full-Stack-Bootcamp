...
app.get('/products', (req, res) => {
  const PRODUCTS = [
    {
      id: 1,
      title: 'Soundcore by Anker Q20i Hybrid Active Noise Cancelling',
      price: '3887.67 INR'
    },
    {
      id: 2,
      title: 'Active Noise Cancelling Headphones,120H Playtime Bluetooth 6.0 Wireless Headphones',
      price: '8553.06 INR'
    },
    {
      id: 3,
...
app.get('/notes', (req, res) => {
  res.sendFile(__dirname + '/class-notes.pdf')
})

app.get('/image', (req, res) => {
  res.sendFile(__dirname + '/nodejs-image.jpg')
})
...
app.get('/students/v1', (req, res) => {
  res.redirect('/students/v2')
})

app.get('/students/v2', (req, res) => {
  const filteredStudents = STUDENTS.filter(s => s.batch == 4)
  res.json(filteredStudents)
})

app.get('/download-notes', (req, res) => {
  res.download(__dirname + '/class-notes.pdf')
})

app.get('/download-image', (req, res) => {
  res.download(__dirname + '/nodejs-image.jpg')
})

app.get('/route-with-issues', (req, res) => {
  try {
    throw new Error('Some wrong logic because of which error occured')
    res.send('All good :)')
  } catch(error) {
...
// 1. /products/cheap-price: Return products whose price < $20
app.get('/products/cheap-price', (req, res) => {
  let filteredProducts = PRODUCTS.filter(p => p.price < 20)
  res.json(filteredProducts)
})

// 2. /products/higher-discount: Return products whose discountPercentage > 12%
app.get('/products/higher-discount', (req, res) => {
  let filteredProducts = PRODUCTS.filter(p => p.discountPercentage > 12)
  res.header('header-1', 'value-1')
  res.header('product-type', 'higher-discount')
  res.json(filteredProducts)
})
...
/*
  # Express.js: Node.js framework
    - .get(): Create an API using which client can read data (HTTP GET request)
    - .post(): Create an API using which client can create data/send data to the server (HTTP POST request)

    - Syntax to create a route:
    ...
    - Response methods:
      - send(): Send any data (text/json)
      - json(): Send only JSON data
      - sendFile(): Send any file
      - redirect(): Redirect to another route
      - download(): Download resource from the server
      - status(): Set HTTP status code for the response
        - 200: OK
        - 404: Not found
        - 500: Internal server error
      - .set()/.header(): Set the header of the response
        - Can be used to set Headers: Metadata for the response (Eg for default headers: date, x-powered-by: "Express", etc.)
        - Headers can be used to send content-type session data, etc.


    - Future:
      - More above HTTP methods and status codes in "RESTful APIs" class
*/